...
static void test_coop_level_activateapp(void)
{
    ...
    skip("Exclusive cooperative-level changes do not reliably send WM_ACTIVATEAPP on macOS ARM64.\n");
#else
        ...
        char *device_name, D3DDEVICEDESC *hal, D3DDEVICEDESC *hel, void *ctx)
{
    ...
    IDirectDraw *ddraw = ctx;

    ok(hal->dwSize == offsetof(D3DDEVICEDESC, dwMinTextureWidth), "Got size %lu.\n", hal->dwSize);
    ok(hel->dwSize == offsetof(D3DDEVICEDESC, dwMinTextureWidth), "Got size %lu.\n", hel->dwSize);
...
static void test_enum_devices(void)
{
    ...
    hr = IDirect3D_EnumDevices(d3d, test_enum_devices_caps_callback, ddraw);
...
START_TEST(ddraw1)
{
    ...
    test_d32_support();
#ifdef __WINE_DARWIN_ARM64_HOST
    skip("Display-mode cursor and window-position tests deadlock the macOS ARM64 display driver.\n");
#else
