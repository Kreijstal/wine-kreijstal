...
static void dcomp_shared_visual_dump( struct object *obj, int verbose )
{
    struct dcomp_shared_visual *visual = (struct dcomp_shared_visual *)obj;

    assert( obj->ops->size == sizeof(*visual) );
    fprintf( stderr, "DirectComposition shared visual process=%p root=%llx\n",
             visual->process, (unsigned long long)visual->target_root );
}

static void dcomp_shared_visual_destroy( struct object *obj )
{
    struct dcomp_shared_visual *visual = (struct dcomp_shared_visual *)obj;

    release_object( visual->process );
}

static const struct object_ops dcomp_shared_visual_ops =
{
    .size = sizeof(struct dcomp_shared_visual),
    .type = &composition_type,
    .dump = dcomp_shared_visual_dump,
    .destroy = dcomp_shared_visual_destroy,
};
...
{
    struct dcomp_shared_visual *visual;

    if (!(visual = alloc_object( &dcomp_shared_visual_ops ))) return;
    visual->process = (struct process *)grab_object( current->process );
    visual->target_root = 0;
    reply->handle = alloc_handle_no_access_check( current->process, visual,
            COMPOSITION_ALL_ACCESS, 0 );
    release_object( visual );
}
...
DECL_HANDLER(dcomp_set_shared_visual_info)
{
    ...
    if (!(visual = (struct dcomp_shared_visual *)get_handle_obj( current->process,
            req->handle, COMPOSITION_MODIFY_STATE, &dcomp_shared_visual_ops ))) return;
    if (visual->process != current->process)
        set_error( STATUS_NOT_IMPLEMENTED );
    else
        visual->target_root = req->target_root;
    release_object( visual );
}
...
DECL_HANDLER(dcomp_get_shared_visual_info)
{
    ...
    if (!(visual = (struct dcomp_shared_visual *)get_handle_obj( current->process,
            req->handle, COMPOSITION_QUERY_STATE, &dcomp_shared_visual_ops ))) return;
    if (visual->process != current->process)
        set_error( STATUS_NOT_IMPLEMENTED );
    else
        reply->target_root = visual->target_root;
    release_object( visual );
}
